"""
LangGraph Kitchen Analysis Agent for AgentCore Runtime
"""
...
class KitchenAnalyzerYOLO:
    def __init__(self, region: str = "us-west-2"):
        ...
        # Load YOLO model for real image analysis using modern ultralytics API
        try:
            from ultralytics import YOLO
            self.yolo_model = YOLO('yolov5s.pt')
            print("✅ YOLO model loaded successfully in AgentCore")
        except Exception as e:
    ...
    def _detect_objects_yolo(self, image_path: str) -> List[DetectedObject]:
        ...
        if not self.yolo_model:
            # Return mock objects for testing when YOLO is not available
            return [
    ...
    def analyze_kitchen(self, image_path: str = None) -> Dict[str, Any]:
        """Analyze kitchen image using LangGraph + Bedrock"""
        print("🔍 Running kitchen analysis with LangGraph + Bedrock...")
        
        # Step 1: YOLO Detection (mock for AgentCore)
        detected_objects = self._detect_objects_yolo(image_path or "mock_image.jpg")
        ...
        return {
            "image_path": image_path or "mock_image.jpg",
...
@app.entrypoint
def langgraph_kitchen_analyzer(payload):
    """
    AgentCore entrypoint for LangGraph kitchen analyzer
    """
    user_input = payload.get("prompt", "")
    image_path = payload.get("image_path", None)
    
    print(f"LangGraph Agent received: {user_input}")
    
    try:
        # Run kitchen analysis
        result = analyzer.analyze_kitchen(image_path)
        ...
        error_result = {
            "error": f"LangGraph analysis failed: {str(e)}",
            "status": "failed"
        }
